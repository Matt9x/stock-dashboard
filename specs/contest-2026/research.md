...
> 2026-09-10 当前Week1接续研究决策见 [week1-recovery-research.md](week1-recovery-research.md)，以下为此前其他研究方向历史。
...
## 6. Week 1 专项研究：十维 PCA 动态赋权重构 NALE

### 6.1 核心研究假说与痛点诊断

在 2025Q3 绿电板块实证回测中，旧版系统显著跑输行业等权 ETF：
1. **行情爆发不知**：在板块主升浪突破期间，算法死守震荡期的波动率上限与固定 $\alpha_{nale}=0.40$，强制做行业等权网络扩散，稀释了突破领涨龙头的强势 Alpha。
2. **静态财报滞后**：传统财务与历史因子存在 90 天财报公布延迟，无法反映最新的市场博弈与产业协同热点。

### 6.2 算法决策与数学建模

- **动态门控机制**：
  将固定的 $\alpha_{nale}=0.40$ 重构为由十维正交文本 PCA 表征驱动的自适应门控：
  $$u_{i,t} = b_m + \sum_{k=1}^{10} w_{k,m} z_{i,k,t}, \quad \alpha_{i,t} = 0.05 + 0.70 \cdot \operatorname{sigmoid}(u_{i,t})$$
  有界范围 $[0.05, 0.75]$ 保障了极值输入的数值稳定性，且 $u=0$ 处精准锚定 $0.40$ 基准线。

- **时间衰减样本加权**：
  采用指数衰减半衰期模型 $\omega_t = 2^{-\text{age}(t)/H}$（默认 $H=60$ 日），让近期最新成熟样本在回归求解中占据更高权重，克服历史数据滞后性。

- **市场状态与网络可靠性交互项 (V4/V5)**：
  - **V4 市场状态项**：$g_t = \text{MA20}_t / \text{MA60}_t - 1$，将大盘趋势注入各分量斜率中 $w_k + v_k g_t$。
  - **V5 网络可靠性项**：$q_t$ 动态度量产业链网络传导的历史误差是否小于单股本征误差，自适应在“相信网络传导”与“相信个股突破”之间动态平衡。

### 6.3 考虑的替代方案及未采纳理由

- ❌ **Softmax 归一化**：被否决。十维权重是资产定价与门控回归参数，可正可负，要求和为 1 会破坏回归的线性自由度与经济学意义。
- ❌ **直接 Ridge 线性回归**：被否决。线性映射无法保证传播系数落在合法区间 $[0.05, 0.75]$ 内，可能出现负传播或无界发散，破坏图扩散的收敛性。
- ❌ **未来收益直接标签化**：被否决。严禁以未来收益或 $(y - S_0)/D$ 作为 $\alpha$ 的监督标签，必须通过公共校准器无量纲超额收益回归联合训练，杜绝前视偏差。
